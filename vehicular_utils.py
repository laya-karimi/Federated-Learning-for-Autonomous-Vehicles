...
def allocate(div_indicator, cost, dataRatesRBs, r_min, Total_RBs):
    ...
    datarates_schedule = []
    while b > 0 and k < len(ordered):
        if ordered[k] < len(dataRatesRBs):
...
def print_solution(prob):
    ...
    for v in prob.variables():
        if v.varValue is not None and v.varValue > 1e-3:
            print(f'{v.name} = {v.varValue}')
    objective_value = value(prob.objective)
    if objective_value is not None:
        print(f"Sum of wts of selected edges = {round(objective_value, 4)}")
    else:
        print("Objective value is None. The problem might not be solved successfully.")


def get_selected_edges(prob):
    selected_from = [v.name.split("_")[1] for v in prob.variables() if v.value() is not None and v.value() > 1e-3]
    selected_to = [v.name.split("_")[2] for v in prob.variables() if v.value() is not None and v.value() > 1e-3]
